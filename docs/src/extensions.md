...
```
Omitting the detector loads every detector of a file into a `NamedTuple` keyed by detector, with a column selection or filter applied to each detector's table:
```julia
...
```
All tiers involved hold one row per trigger of the same detector, so their rows correspond by position; a row-count mismatch raises a `DimensionMismatch`. Without a detector, every detector of the file is filtered by its own rows in the filter tiers.
